...
for Si_Do in level1_list:
  df_delta = df[df['지역']==Si_Do]
  for row in df_delta.itertuples():    
    cursor.execute('''INSERT INTO deltacorona_by_area1 (area1, year_month, delta) VALUES(?, ?, ?)''', 
    [Si_Do,  row.year_month, row.전일대비증감수])
