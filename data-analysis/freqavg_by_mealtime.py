...
for area1 in level1_list:
  ...
  for time in peakTime_list:
    ...
    peak_df = peak_df.groupby(['광역시도','시군구', '읍면동'])['시간대','배달건수'].mean().sort_values(by='배달건수',ascending=False).reset_index().head(3)
...
'''
테이블에 넣고 싶은 데이터를 데이터프레임 하나에 모은 뒤 to_sql을 이용해서 바로 테이블에 넣는 것은 어떨까?
'''