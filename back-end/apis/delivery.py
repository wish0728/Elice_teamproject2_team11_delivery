...
@deliveryfreq.route('/getFreq/<int:year>/<string:area1>/<string:area2>')
@deliveryfreq.response(200, "Found")
@deliveryfreq.response(404, "Not found")
@deliveryfreq.response(500, "Internal Error")
class getFreq(Resource): 
    def get(self,year,area1,area2):
        ...
        query = d.query.filter_by(area1_City_Do=area2)
...
# 슬아님 방법 적용한 평균(func.avg) - 일단 실패 
# @deliveryfreq.route('/getFreqTest/<string:name>')
# @deliveryfreq.response(200, "Found")
# @deliveryfreq.response(404, "Not found")
# @deliveryfreq.response(500, "Internal Error")
# class getFreq(Resource): 
...
#         rows = db.session.query(d.time,func.avg(d.delivery_freq)).filter(d.area2_Si_Gun_Gu==name).all()
#         data={}
#         name={}
#         for row in rows:
#             time = row[0]
